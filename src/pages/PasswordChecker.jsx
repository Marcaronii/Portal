...
export default function PasswordChecker() {
  ...
  function check() {
    ...
    let strength, tone;
    if (password.length < 6) { strength = "Weak"; tone = "bad"; }
    else if (password.length <= 9) { strength = "Medium"; tone = "warn"; }
    else { strength = "Strong"; tone = "good"; }

    const status =
      strength === "Strong"
        ? "Strong — you can use this password."
        : "Weak — create a stronger password.";

    setError("");
    setResult({ strength, status, tone });
  }
  ...
  const barColor = { Weak: CORAL, Medium: AMBER, Strong: TEAL };

  return (
    <div>
      <Navbar />
      <Shell number="03" title="Password Strength Checker" subtitle="Activity 3">
        ...
        <PrimaryButton onClick={check}>Check Password</PrimaryButton>
        <SecondaryButton onClick={clear}>Clear</SecondaryButton>
        {result && (
          <Output tone={result.tone}>
            <OutputRow label="Strength" value={result.strength} />
            <div className="text-slate-300 pt-1">{result.status}</div>
            <div className="w-full h-1.5 rounded-full overflow-hidden mt-2" style={{ backgroundColor: "#2A3452" }}>
              <div className={`h-full rounded-full ${barWidth[result.strength]}`} style={{ backgroundColor: barColor[result.strength] }} />
            </div>
          </Output>
        )}
      </Shell>

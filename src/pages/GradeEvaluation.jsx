...
export default function GradeEvaluation() {
  ...
  function evaluate() {
    ...
    let remarks, tone;
    if (s >= 90) { remarks = "Excellent"; tone = "good"; }
    else if (s >= 85) { remarks = "Very Good"; tone = "good"; }
    else if (s >= 80) { remarks = "Good"; tone = "warn"; }
    else if (s >= 75) { remarks = "Passed"; tone = "warn"; }
    else { remarks = "Failed"; tone = "bad"; }

    setError("");
    setResult({ name, score: s, remarks, tone });
  }
  ...
  return (
    <div>
      <Navbar />
      <Shell number="02" title="Student Grade Evaluation" subtitle="Activity 2">
        <Field label="Student Name">
          <input className={inputCls} style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter student name" />
        </Field>
        <Field label="Score">
          <input className={inputCls} style={inputStyle} value={score} onChange={(e) => setScore(e.target.value)} placeholder="Enter score (0-100)" />
        </Field>
        <ErrorBox message={error} />
        <PrimaryButton onClick={evaluate}>Evaluate</PrimaryButton>
        <SecondaryButton onClick={clear}>Clear</SecondaryButton>
        {result && (
          <Output tone={result.tone}>
            <OutputRow label="Student" value={result.name} />
            <OutputRow label="Score" value={result.score} />
            <OutputRow label="Remarks" value={result.remarks} />
          </Output>
        )}
      </Shell>
    </div>
